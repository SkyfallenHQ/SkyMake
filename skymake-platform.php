...
if(substr( $request, 0, 7 ) === "lesson/") {
    ...
    if (!($lessonname == "n")) {
        ...
        echo(overview(getassignedlessons($link)[$n], getassignedteachers($link)[$n], getassignedtimes($link)[$n], getassignedtopics($link)[$n], getassignedunits($link)[$n], getassignedbgurls($link)[$n],getassignedids($link)[$n],"<p>No content assigned.</p>"));
    }else {
